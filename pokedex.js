...
function populatePokedex(data) {
  ...
  for (var id = 1; id <= 151; id++) {
    pokedexListContainer.appendChild(
      createPokedexListItem(data.pokemon[id - 1], data.captureInfo[id]),
    );
  }
...
function createPokedexListItem(pokemon, captureData) {
  ...
  itemIdContainer.innerHTML =
    "<p>" + applyPokemonIdPadding(pokemon.id) + "</p>";
  ...
  if (captureData) {
    ...
    itemPokeballContainer.className = itemPokeballContainer.className +=
      " caught";
    ...
    item.onclick = function () {
      onPokedexEntryClick(pokemon, captureData);
    };
  }
...
function populateTrainerStats(level, caught) {
  var pokedexTrainerStatsContainer = document.getElementById(
    "pokedex-trainer-details-container",
  );

  pokedexTrainerStatsContainer.innerHTML =
    "<p>LEVEL. " + level + "<br><p>OWN " + caught + "/151</p>";
}
...
function onPokedexEntryClick(pokemon, captureData) {
  var spriteURL = chrome.runtime.getURL(
    "resources/sprites/" + pokemon.id + ".png",
  );
  ...
  var info =
    "<p>HIGHEST Lv. " +
    captureData.highestlvl +
    "</p><p>CAUGHT." +
    captureData.count +
    '</p><p class="poke-info">' +
    pokemon.info +
    "</p>" +
    '</p><p class="poke-info">TYPE. <b>' +
    pokemon.types.join(" ,") +
    "</b></p>";
