...
function PokedexRepository() {
  ...
  var calculateTrainerLevel = function () {
    ...
    for (var id = 1; id <= 151; id++) {
      ...
      if (captureDetails) pokemonHighestLevels.push(captureDetails.highestlvl);
    }
    // Sort the level list smallest to highest.
    pokemonHighestLevels = pokemonHighestLevels.sort(function (a, b) {
      return a - b;
    });
    // Restrict the list to the highest 10.
    pokemonHighestLevels = pokemonHighestLevels.slice(
      pokemonHighestLevels.length - 10,
    );
  ...
  this.addCaughtPokemon = function (id, level) {
    ...
    if (captureDetails) {
      ...
      captureDetails.highestlvl =
        level > captureDetails.highestlvl ? level : captureDetails.highestlvl;
